...
public:
    ...
    //// Methods (pblc):
    void AddBrick(
            /**/const/**/ CAniObject* const baseBrick,  //@@ 1й `const` возможно убрать, т.к. в методе инициализирую неконстантный объект. Ск.вс. понадобится конст_каст
...
///@@@ Чего так??: (я ж подключил хедер, в котором typedef)
///@@@ error C2065: '_LAV_HANDLER' : undeclared identifier             
///@@@..    std::auto_ptr<_LAV_HANDLER> GetLavHndlr(const u32 key);     //интерфейсный метода, возвращающий m_LavBricks.CreateAccessHandler(key).   ///@@@Просмотреть, применяю где-то, или нет
//    std::auto_ptr</*CLockableAccessVec<_BRICK*>::CLavHandler*/_LAV_MACRO> GetLavHndlr(const u32 key);     //интерфейсный метода, возвращающий m_LavBricks.CreateAccessHandler(key).   ///@@@Просмотреть, применяю где-то, или нет
    std::auto_ptr<_LAV_OFBRICKS_HANDLER> GetLavHndlr(const u32 key);     //интерфейсный метода, возвращающий m_LavBricks.CreateAccessHandler(key).   ///@@@Просмотреть, применяю где-то, или нет
    void CleanBricks(                           //перебирает кирпичи из `m_LavBricks` и применяет `DelBrick()`
            CScene* const                   sc,
            CArkanoidController* const      pArkCntrllr
        );
    ...
    void StartAddBricks(    //Заполняет вектор кирпичей и размещает их в сцене. (применяется на старте игры и (только в случае варианта реализации с удалением ани-биков) также после выигрыша)
            CScene*                 sc, 
