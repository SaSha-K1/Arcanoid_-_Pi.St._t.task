...
{
    float newChckPnt;
    float inaccuracy = 0.00001;  //компенсация погрешности float
    m_svfBallHitChckPntsAngles.push_back(0.0);
    for (u32 i=1;  i < m_suNUM_OF_HIT_CHCK_PNTS;  ++i)  //начинаю с 1, т.к. 0-вой эл-т инициализирую перед циклом
    {
        newChckPnt = 2 * PI * i / m_suNUM_OF_HIT_CHCK_PNTS;
        if (newChckPnt-inaccuracy > PI)     //Конвертирую угол их диапазона [0; 2*PI) в диапазон (-PI; PI].     
                                            //.. Тут можно было использовать и NormalizeFi(), если переписать её, чтобы принимала ..
                                            //.. параметр, но действие простое, а в NormalizeFi() больше проверок и действий. Решил, ..
