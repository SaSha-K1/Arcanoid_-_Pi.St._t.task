...
const u32               _BALL::m_suNUM_OF_HIT_CHCK_PNTS = 8;    //Сейчас ==8 : (N(north),E(east),S,W + 4 промежуточных). ..
                                                                //.. !В случае изменения, увязать с новым знач-ем: enum DIRECTION и ..
...
std::map<_BALL::DIRECTION,vector2>  _BALL::m_smFewPixelsShift;
...
//{
//    std::vector<float> vfBallHitChckPntsAngles;   ///@@@ ? актуальна ли эта инфа?: //#IMPORTANT: в случ.необходимости изменения кол-ва эл-тов, менять только на чётное кол-во >=2. Увязать с новым знач-ем enum DIRECTION и class CBall static member m_suNUM_OF_HIT_CHCK_PNTS.
...
//// ComputeNewFi() ////
void _BALL::ComputeNewFi(const DIRECTION dir)
{
...
//// GetNewMoveCenter() ////
vector2 _BALL::GetNewMoveCenter(const DIRECTION dir)
{
#if DEBUG==1
...
//Это рассчёт поправки (m_sfFiPlus) к углу отражения (m_fFi) в зависимости от удаления точки контакта с Pad'ом от середины последнего.
{
    ...
    m_sfFiPlus = (PI/2) * weightedDlt * ( (fabs(m_fFi)-PI/2) / PI / 2 );	//Максимальная корректировка к углу == PI/4, корректируется ..
...
void _BALL::IsHitWithScreenBorders(
        const float             fSceneW,
        std::vector<DIRECTION>& rsltTrggrdHitChckPnts  //модифицируемый vector Triggered Hit Check Points - сработавшие датчики контакта
    ) const
{
     ...
     )
{
    ...
    else
    {
        ...
        {                               //#OUTDATED (было актуально д/ диапазона выраженного в DIRECTION): #WARNING(possibly). Возможно заменить implicit преобразование типов явным.
            if ( pPad->Hit(m_svHitChckPnts[d] + Vec3To2(m_pBallPicObj->GetPos())/*,0*/) )
            {
                rsltTrggrdHitChckPnts.push_back(static_cast<DIRECTION>(d));
...
//#endif //LOCKS
        ...
        for (char ib = 0;  ib < (*pLavHndlr)->size();  ++ib)	//Костыли
        {		
            ...
            if (m_svHitChckPnts[S].y + m_pBallPicObj->GetPos().y < ao.GetPos().y -fDltPxlsToChckHit  ||	
                m_svHitChckPnts[N].y + m_pBallPicObj->GetPos().y > ao.GetPos().y + ao.GetSize().y +fDltPxlsToChckHit || 
            ...
            {
                if ( /*ao->*/ao.Hit(m_svHitChckPnts[i] + Vec3To2(m_pBallPicObj->GetPos())/*,0*/) )
                {
                    rsltTrggrdHitChckPnts.push_back(static_cast<DIRECTION>(i));
                    if (/*(*it)*/ (*(*pLavHndlr))[ib]/**/->m_state == _BRICK::ING)
                        /*(*it)*/ (*(*pLavHndlr))[ib]/**/->m_state=_BRICK::DSTR_ST;
                }
            }
        }
//#OUTDATED: #if LOCKS==1
...
_BALL::DIRECTION  _BALL::InterpretIsHitResults(std::vector<DIRECTION>& vTrggrdHitChckPnts) const
{
...
//Поиск столкновения с Pad'ом
    IsHitWithPad(pPad, pPad->GetCenter().y - pCurrBall->GetCenter().y, fDltPxlsToChckHit, vTrggrdHitChckPnts, needToCheckBricks);
...
//#UGLY: попробовать уйти от такого длинного обращения к типу DIRECTION может ч/з typedef - читал в теме nested classes, что typedef вроде должен быть виден и вовне. Ну в крайн.случ. попробовать использовать суффикс-синтаксис ("auto .. )-> ...")
_BALL::DIRECTION  _BALL::HitReview(DIRECTION dir) const
{
#if DEBUG==1
...
void _BALL::InitFewPixelsShift()
{
    ...
    for (u32 i=0; i < m_svHitChckPnts.size(); ++i)
    {
        ...
        m_smFewPixelsShift[static_cast<DIRECTION>(i)] = vector2(   //#COMPILER(type_cast)
